...
In integration mode, item images are served through signed `/api/bjpkeep/image` proxy URLs generated by the Home Assistant integration. If image requests return `401 Unauthorized`, confirm the installed integration version is `0.1.1a` or newer, then restart Home Assistant and refresh the dashboard.
